...
# NOTE: the "executive CFO report" was removed. It built a dollar figure from
# invented constants (an SLA rate of $250/min and a 12.5-minute average outage)
# and printed "100% verification rate" / "0 flap oscillations" as literals rather
# than measuring them. Cost claims here are now limited to what the loop actually
# observes: capacity units returned during COOLDOWN, valued via `cost_per_unit_hr`.
# For evidence that an outage was avoided, use CHRONO-PROOF (`proof.py`), which
# reports breach-seconds avoided from real SigNoz telemetry.
