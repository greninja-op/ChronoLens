...
# NOTE: the old `generate_counterfactual_projection` was removed. It drew a
# synthetic "what would have happened" curve from a formula, with no telemetry
# behind it. The real, SigNoz-measured counterfactual now lives in `proof.py`
# (CHRONO-PROOF), which fits the pre-action trend from actual p99 samples and
# overlays the measured post-action reality.
