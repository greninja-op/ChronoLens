...
# --------------------------------------------------------------------------- #
# Stitch AI Data Pipeline Endpoints
# --------------------------------------------------------------------------- #
@app.post("/webhook/stitch")
async def stitch_webhook(request: Request):
    """Inbound webhook handler for Stitch AI pipeline triggers."""
    from chronolens.stitch import process_stitch_webhook
    try:
        body = await request.json()
    except Exception:
        body = {}
    res = process_stitch_webhook(body, cfg=cfg)
    return res


@app.post("/api/stitch/test")
def stitch_test_stream():
    """Trigger a test event stream synchronization to Stitch AI."""
    from chronolens.stitch import stream_event_to_stitch
    test_payload = {
        "service": "checkout-service",
        "p99_ms": 480.0,
        "slope_ms_per_s": 18.5,
    ...
    res = stream_event_to_stitch("test_sync", test_payload, cfg=cfg)
    return {"ok": True, "stitch_response": res}


# --------------------------------------------------------------------------- #
# Google Stitch AI MCP (Model Context Protocol) Server Endpoint
# Project: https://stitch.withgoogle.com/projects/13586600990628684692
# --------------------------------------------------------------------------- #
@app.post("/mcp/stitch")
@app.post("/api/mcp/stitch")
async def stitch_mcp_server(request: Request):
    """MCP (Model Context Protocol) server endpoint for Google Stitch AI UI connection."""
    try:
        body = await request.json()
    except Exception:
        body = {}
    ...
    # MCP Protocol Dispatcher for Google Stitch AI
    if method == "tools/list":
        return {
            ...
            "result": {
                "tools": [
                    {
                        "name": "get_p99_forecast",
                        "description": "Returns ChronoLens AI slope forecast and ETA to breach for checkout services.",
                        "inputSchema": {"type": "object", "properties": {}},
                    },
                    {
                        "name": "get_counterfactual_timelines",
                        "description": "Returns Chrono-Replay parallel counterfactual timelines (Timeline A vs Timeline B).",
                        "inputSchema": {"type": "object", "properties": {}},
                    },
                    {
                        "name": "run_reliability_loop",
                        "description": "Executes ChronoLens pre-emptive PREVENT -> VERIFY -> COOLDOWN reliability loop.",
                        "inputSchema": {"type": "object", "properties": {}},
                    },
                ]
            },
            ...
            return {
                ...
                "result": {
                    "content": [
                        {
                            "type": "text",
                            "text": f"Service {fc.service} p99: {fc.current_p99_ms}ms, trend slope: +{fc.slope_ms_per_s}ms/s, breach ETA: {fc.seconds_to_breach}s.",
                        }
                    ]
                },
            }
        elif tool_name == "get_counterfactual_timelines":
            from chronolens.foresee import generate_counterfactual_projection
            data = generate_counterfactual_projection(cfg)
            return {
                "jsonrpc": "2.0",
                "id": msg_id,
                "result": {"content": [{"type": "text", "text": str(data)}]},
            }
        elif tool_name == "run_reliability_loop":
            ...
            return {
                ...
                "result": {
                    "content": [
                        {
                            "type": "text",
                            "text": f"Reliability loop executed for {case.service}: outcome={case.outcome}, dollars_saved=${case.dollars_saved:.2f}",
                        }
                    ]
                },
            }

    # Default MCP capabilities / handshake
    return {
        "jsonrpc": "2.0",
        "id": msg_id,
        "result": {
            ...
            "capabilities": {"tools": {}},
            "serverInfo": {
                "name": "ChronoLens Stitch MCP Server",
                "version": "1.0.0",
                "stitch_project_url": "https://stitch.withgoogle.com/projects/13586600990628684692",
            },
        },
    }
...
def stitch_status():
    """Retrieve Stitch AI integration connection status."""
    return {
        "enabled": cfg.stitch_enabled(),
        "workspace_id": cfg.stitch_workspace_id,
        "project_id": cfg.stitch_project_id,
        "project_url": cfg.stitch_project_url,
        "mcp_endpoint": "http://localhost:8095/mcp/stitch",
        "webhook_url": cfg.stitch_webhook_url,
        "api_key_present": bool(cfg.stitch_api_key),
    }
